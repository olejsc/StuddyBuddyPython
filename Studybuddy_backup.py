...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        def setupActionsObjects(self, MainWindow):
            ...
            self.tray_icon.lecture_action = QtWidgets.QAction("Lecture mode", MainWindow)
...
# just the setup, but also during event handling and actions.
    ...
    def showTrayMessage(self, value):
        self.tray_icon.showMessage(
            value['SubjectCode'],
            value['Name']+"\n" + "Notication was automatically postponed.",
            QSystemTrayIcon.Information,
            2000
        )
...
# effectively postponing the notification.
        ...
        if self.settings_checkbox_lecture_mode.isChecked() is False:
            ...
            if msg_box.clickedButton() == yes_button:
                ...
                now = now = datetime.now().strftime("%y-%m-%d-%H-%M-%S")
                self.set_notification_key_value(key, value['Name'], now)
                self.create_postponed_thread(value)
        else:
            self.showTrayMessage(value)
            self.create_postponed_thread(value)


    def create_postponed_thread(self,value):
        random_string = ''.join([random.choice(string.ascii_letters + string.digits) for n in range(10)])
...
# Writes the notification to file, appending it to other, existing notifications (if any).
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.settings_checkbox_lecture_mode.setText(_translate("MainWindow", "Lecture mode"))
...
class Worker(QObject):
    ...
    def work(self):
        ...
        while self.isRunning == True:
            for x in range (0,15000 + 300,300):
